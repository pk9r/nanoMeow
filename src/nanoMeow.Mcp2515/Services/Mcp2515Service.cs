...
namespace nanoMeow.Mcp2515.Services
{
    ...
    public class Mcp2515Service : IDisposable
    {
            ...
            GpioController gpioController)
        {
            ...
            PinSpiMcuMcp2515 = pinSpiMcuMcp2515;
            GpioController = gpioController;
            var spiConnectionSettings =
                new SpiConnectionSettings(PinSpiMcuMcp2515.BusId, PinSpiMcuMcp2515.CS)
                {
            ...
            Mcp2515 = new Iot.Device.Mcp25xxx.Mcp2515(
                spiDevice: SpiDevice,
                //interrupt: PinSpiMcuMcp2515.INT, // Mcp25xxx bad open interrupt
                shouldDispose: true
            );
            ...
            if (interruptPin != -1)
            {
                _logger.LogInformation("Open pin {0} as interrupt pin", interruptPin);
                Interrupt = GpioController.OpenPin(interruptPin, PinMode.Input);
        ...
        private void HandleInterrupt(
            object sender, PinValueChangedEventArgs e)
        {
            _logger.LogInformation("Interrupt.ValueChanged {0}", sender);
        }
        ...
        public void Reset()
        {
            ...
            var zeros = new SpanByte(new byte[14]);
            Mcp2515.Write(Address.TxB0Ctrl, zeros);
            Mcp2515.Write(Address.TxB1Ctrl, zeros);
            Mcp2515.Write(Address.TxB2Ctrl, zeros);

            Mcp2515.WriteByte(Address.RxB0Ctrl, 0);
            Mcp2515.WriteByte(Address.RxB1Ctrl, 0);

            Mcp2515.WriteByte(Address.CanIntE,
                (byte)(Mcp2515CanIntF.RX0IF | Mcp2515CanIntF.RX1IF |
                Mcp2515CanIntF.ERRIF | Mcp2515CanIntF.MERRF));

            // receives all valid messages using either Standard or Extended Identifiers that
            // meet filter criteria. RXF0 is applied for RXB0, RXF1 is applied for RXB1
            Mcp2515.BitModify(
                address: Address.RxB0Ctrl,
                mask: (byte)Mcp2515RXBnCtrl.RXM_MASK | (byte)(Mcp2515RxB0Ctrl.BUKT | Mcp2515RxB0Ctrl.FILHIT_MASK),
                value: (byte)Mcp2515RXBnCtrl.RXM_STDEXT | (byte)(Mcp2515RxB0Ctrl.BUKT | Mcp2515RxB0Ctrl.FILHIT)
            );
            Mcp2515.BitModify(
                address: Address.RxB1Ctrl,
                mask: (byte)Mcp2515RXBnCtrl.RXM_MASK | (byte)Mcp2515RxB1Ctrl.FILHIT_MASK,
                value: (byte)Mcp2515RXBnCtrl.RXM_STDEXT | (byte)Mcp2515RxB1Ctrl.FILHIT
            );

            // clear filters and masks
            // do not filter any standard frames for RXF0 used by RXB0
            // do not filter any extended frames for RXF1 used by RXB1
            var filters = new Mcp2515Rxf[] { Mcp2515Rxf.RXF0, Mcp2515Rxf.RXF1, Mcp2515Rxf.RXF2, Mcp2515Rxf.RXF3, Mcp2515Rxf.RXF4, Mcp2515Rxf.RXF5 };
            ...
            var masks = new Mask[] { Mask.MASK0, Mask.MASK1 };
        ...
        private readonly Mcp2515RXBnRegs[] RXB = new Mcp2515RXBnRegs[]
        {
            new()
            {
                Ctrl = Address.RxB0Ctrl, Sidh = RxBufferAddressPointer.RxB0Sidh,
                Data = RxBufferAddressPointer.RxB0D0, RXnIF = Mcp2515CanIntF.RX0IF
            },
            new()
            {
                Ctrl = Address.RxB1Ctrl, Sidh = RxBufferAddressPointer.RxB1Sidh,
                Data = RxBufferAddressPointer.RxB1D0, RXnIF = Mcp2515CanIntF.RX1IF
            }
        };
        ...
        private void SendMessage(Mcp2515TXBn txbn, CanFrame frame)
        {
            if (frame.CanDlc > CAN_MAX_DLEN)
                throw new Exception("Data length exceeds maximum CAN frame size");

            var txbuf = TXB[(byte)txbn];

            var data = new SpanByte(new byte[13]);

            bool ext = (frame.CanId & CAN_EFF_FLAG) != 0;
            bool rtr = (frame.CanId & CAN_RTR_FLAG) != 0;
            uint id = (uint)(frame.CanId & (ext ? CAN_EFF_MASK : CAN_SFF_MASK));
            ...
            {
                throw new Exception("Error occurred during CAN message transmission");
            }
        }
        ...
        public void SendMessage(CanFrame frame)
        {
            ...
            var txBuffers = new Mcp2515TXBn[] { Mcp2515TXBn.TXB0, Mcp2515TXBn.TXB1, Mcp2515TXBn.TXB2 };
            ...
            throw new Exception("All transmit buffers are full");
        }
        ...
        public CanFrame ReadMessage()
        {
            ...
            throw new Exception("ERROR_NOMSG");
        }

        private void SetFilter(Mcp2515Rxf num, bool ext, uint ulData)
        {
            SetConfigMode();
            var reg = num switch
            {
                ...
                _ => throw new Exception("Invalid filter number"),
            };
        ...
        private void SetFilterMask(Mask mask, bool ext, uint ulData)
        {
            SetConfigMode();
            ...
            var reg = mask switch
            {
                Mask.MASK0 => Address.RxM0Sidh,
                Mask.MASK1 => Address.RxM1Sidh,
        ...
        private void SetMode(Mcp2515Mode mode, int milisTimeout = 100)
        {
            if (IsInMode(mode))
                return;
            ...
            var endTime = DateTime.UtcNow.Ticks +
                milisTimeout * TimeSpan.FromMilliseconds(1).Ticks;
            ...
            if (!modeMatch)
                throw new Exception("Failed to enter requested mode");

            _logger.LogInformation(
                "MCP2515 mode set to {0} mode", mode);
        }

        private bool IsInMode(Mcp2515Mode mode)
        {
            var currentMode = Mcp2515.Read(Address.CanStat);
            currentMode &= CANSTAT_OPMOD;
            return currentMode == (byte)mode;
        }

        public void SetNormalMode(int milisTimeout = 100)
        {
            SetMode(Mcp2515Mode.Normal, milisTimeout);
        }

        public void SetConfigMode(int milisTimeout = 100)
        {
            SetMode(Mcp2515Mode.Configuration, milisTimeout);
        }

        public void SetLoopbackMode(int milisTimeout = 100)
        {
            SetMode(Mcp2515Mode.Loopback, milisTimeout);
        }
